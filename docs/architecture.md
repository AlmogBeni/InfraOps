...
4. **Worker** claims the job, materialises the 21 step rows from the ordered registry and
...
configure_hardware → attach_network_adapter → power_on → wait_for_tools →
...
|---|---|---|
| `SecretsProvider` | env, vault | lazy resolution, never logged |
