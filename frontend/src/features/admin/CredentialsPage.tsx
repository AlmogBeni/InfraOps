...
import { CalendarClock, Database, KeyRound, LockKeyhole, Pencil, Plus, ShieldCheck, Trash2 } from 'lucide-react'
...
const PROVIDER_META = {
  database: {
    label: 'InfraOps encrypted storage',
    description: 'Encrypted at rest and refreshed from the backend for every operation.',
    icon: Database,
  },
} as const
...
function providerMeta(provider: string) {
  return PROVIDER_META[provider as keyof typeof PROVIDER_META] ?? {
    ...
    description: 'Edit this legacy reference and save both values to move it into encrypted storage.',
    icon: Database,
  }
}
...
export function CredentialsPage() {
  ...
  const [editing, setEditing] = useState<SecretReferenceOut | null>(null)
  ...
  const create = useMutation({
    mutationFn: () => editing
      ? api.admin.updateCredential(editing.id, {
          purpose: form.purpose,
          description: form.description,
          ...(form.username ? { username: form.username } : {}),
          ...(form.password ? { password: form.password } : {}),
        })
      : api.admin.createCredential({
          ...form,
          name: normalizedReferenceName,
          provider: 'database',
        }),
    onSuccess: () => {
      setDialogOpen(false)
      setEditing(null)
      setForm({ name: '', purpose: 'generic', username: '', password: '', description: '' })
      setFormError(null)
  ...
  const managedCount = items.filter((item) => item.provider === 'database').length
  ...
  function openEditDialog(credential: SecretReferenceOut) {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        title="Credentials"
        description="Manage encrypted credential pairs used by infrastructure connections and provisioning workflows."
        actions={<Button size="sm" onClick={openCreateDialog}><Plus className="h-4 w-4" /> Add credential</Button>}
        meta={(
          <>
            <span>{items.length} credential{items.length === 1 ? '' : 's'}</span>
            <span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-3.5 w-3.5 text-brand-700" /> Encrypted at rest</span>
          </>
        )}
      />
      ...
      <section className="animate-panel-reveal overflow-hidden rounded-2xl border border-[#285f50] bg-[#173f34] text-white shadow-[0_18px_44px_rgba(23,63,52,0.18)]" aria-label="Credential security model">
        <div className="relative grid gap-4 px-5 py-5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center sm:px-6">
          ...
          <div className="relative flex items-start gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold tracking-[-0.01em]">Backend-managed credentials</h2>
              <p className="mt-1 max-w-2xl text-xs leading-5 text-white/65">
                Values are encrypted with the application root key, never returned by the API, and reloaded for every operation.
              </p>
            </div>
          </div>
          <div className="relative flex flex-wrap gap-2 sm:justify-end">
            <Badge className="bg-white/10 text-white ring-white/15">{managedCount} managed</Badge>
            <Badge className="bg-[#d8f06a]/12 text-[#e9f8a6] ring-[#d8f06a]/25">{configuredCount} ready</Badge>
          </div>
        </div>
      </section>
      ...
      {credentials.isLoading ? (
        <LoadingState title="Loading credentials" description="Retrieving current encrypted credential metadata." />
      ) : credentials.isError ? (
        <Alert tone="danger" title="Credential store unavailable">
        ...
        <EmptyState
          ...
          description="Add an encrypted credential before configuring vCenter or guest provisioning."
          action={<Button size="sm" onClick={openCreateDialog}><Plus className="h-3.5 w-3.5" /> Add credential</Button>}
        />
      ) : (
              ...
              >
                ...
                <div className="flex min-h-56 flex-col p-5">
                  <div className="flex items-start gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex flex-wrap gap-1.5">
                        <Badge tone="info">{provider.label}</Badge>
                        <Badge tone={credential.configured ? 'success' : 'warning'}>{credential.configured ? 'Ready' : 'Needs rotation'}</Badge>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-4 rounded-xl border border-[#e1e7df] bg-[#f7faf5] px-3.5 py-3">
                    <p className="flex items-center gap-2 text-[11px] font-semibold text-[#355047]"><ShieldCheck className="h-3.5 w-3.5 text-brand-700" /> {humanizeIdentifier(credential.purpose)}</p>
                    <p className="mt-1 text-[10px] leading-4 text-[#78837c]">{provider.description} Revision {credential.revision}.</p>
                  </div>
                  ...
                  <footer className="mt-auto flex items-end justify-between gap-3 border-t border-[#e7eae6] pt-4">
                    ...
                    <Button size="sm" variant="ghost" onClick={() => openEditDialog(credential)}>
        ...
        footer={(
          <>
            ...
            <Button
              ...
              disabled={!isValidSecretReference(normalizedReferenceName) || (!editing && (!form.username || !form.password))}
      ...
      >
        ...
        <div className="mb-5 flex items-start gap-3 rounded-xl border border-brand-100 bg-brand-50/60 p-3.5">
          ...
          <div><p className="text-xs font-semibold text-brand-900">Encrypted credential storage</p><p className="mt-1 text-[11px] leading-4 text-brand-800/70">Values are encrypted before they are stored and are never returned by the API.</p></div>
        </div>
        ...
        <FormRow label="Purpose" htmlFor="cred-purpose" required>
          <Select
            id="cred-purpose"
            value={form.purpose}
            onChange={(event) => { setFormError(null); setForm({ ...form, purpose: event.target.value }) }}
          >
            <option value="generic">Generic credential</option>
        ...
        <Alert tone="info" title="Rotations apply live">
          Running services resolve the latest encrypted revision from the backend for each new operation; no container restart is required.
        </Alert>
      </Dialog>
