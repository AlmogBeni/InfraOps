...
export function NetworkStep() {
  ...
  return (
    <section aria-label="Network configuration" className="space-y-4">
      <header>
        <h2 className="text-sm font-semibold text-slate-800">Networking</h2>
        <p className="text-xs text-slate-500">
          VMware port group and guest OS IP configuration are applied automatically during provisioning.
        </p>
      </header>
      ...
      <div className="grid grid-cols-1 gap-x-4 sm:grid-cols-2">
          ...
          >
            ...
            {(networks.data ?? []).map((network) => (
              <option key={network.id} value={network.id}>
                {network.name}{' '}
                {network.type === 'DISTRIBUTED_PORT_GROUP' ? '(distributed)' : '(standard)'}
              </option>
            ))}
          </Select>
        </FormRow>
        ...
        <FormRow label="Adapter type" htmlFor="adapter-type">
          <Select
            ...
            onChange={(event) =>
              wizard.update({ adapter_type: event.target.value as 'VMXNET3' | 'E1000E' })
            }
          >
          ...
          <div className="grid grid-cols-1 gap-x-4 sm:grid-cols-3">
            <FormRow label="IP address" htmlFor="ip-address" required error={wizard.errors['ip_address']}>
              <Input
                ...
                onChange={(event) => wizard.update({ ip_address: event.target.value })}
              />
            </FormRow>
            ...
            <FormRow label="Default gateway" htmlFor="ip-gateway" required error={wizard.errors['gateway']}>
              <Input
                ...
                className="font-mono"
                value={data.gateway}
                onChange={(event) => wizard.update({ gateway: event.target.value })}
              />
            </FormRow>
            ...
            <FormRow label="Primary DNS" htmlFor="dns-primary" required error={wizard.errors['dns_primary']}>
              <Input
                ...
                onChange={(event) => wizard.update({ dns_primary: event.target.value })}
              />
            </FormRow>
            ...
            <FormRow label="Secondary DNS" htmlFor="dns-secondary">
              <Input
                ...
                onChange={(event) => wizard.update({ dns_secondary: event.target.value })}
              />
            </FormRow>
          ...
          <div className="rounded-md border border-slate-200 p-3">
            <div className="flex items-center justify-between gap-2">
              ...
              <Button
                ...
                size="sm"
                variant="secondary"
                loading={ipCheck.isPending}
                disabled={!data.ip_address}
                onClick={() => ipCheck.mutate()}
              >
                Check address now
              </Button>
            </div>
              ...
              <div className="mt-2 space-y-1.5">
                {conflictReport.providers.map((provider) => (
                  <div key={provider.provider} className="flex items-center gap-2 text-xs">
                    <Badge
                      tone={
                        provider.status === 'CONFLICT_DETECTED'
                          ? 'danger'
                          : provider.status === 'NO_CONFLICT'
                            ? 'success'
                            : 'neutral'
                      }
                    >
